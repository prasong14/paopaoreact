...
if (isset($_POST['cmd']) && $_POST['cmd'] == 'save') {
  ...
  $sql = "SELECT * FROM dept_emp WHERE emp_no = '$emp_no' AND dep_no = '$dep_no' ORDER BY to_date DESC";
  $result = mysqli_query($conn, $sql);
  if (mysqli_num_rows($result) > 0) {
    // he is still in the same department, no department change
  } else {
    // 1. terminate the current department to_date = today; 
    $sql = "UPDATE dept_emp SET to_date = NOW() WHERE emp_no = '$emp_no' AND dep_no = '$dep_no' AND to_date = '9999-01-01'";
    if ($conn->query($sql) === TRUE) {
      echo "Success<br/>$sql<br/>";
    } else {
      echo "Error: " . $sql . "<br/>" . $conn->error;
